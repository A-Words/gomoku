...
Game.run()
  |
  +-- Board       棋盘状态、落子、悔棋、满盘判断
  +-- rules       五子连珠判定、平局判定
  +-- Renderer    棋盘、棋子、菜单、按钮、提示绘制
  +-- GameRecord  棋谱记录、保存、加载
  +-- ReplayController 棋谱回放控制
  +-- AI          人机对战落子决策
```
...
轮到 AI
  |
  v
生成候选点 -> 紧急着法检测 -> Minimax 搜索 -> 返回最佳落子
  |
  v
AI 落子并再次判定胜负
```
